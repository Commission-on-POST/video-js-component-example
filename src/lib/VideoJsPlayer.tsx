import {
  ...
  FunctionalityRemover,
  IFuncParams,
} from "@commission-on-post/post-cat-functionality-tool";
...
const defaultProps: VideoJsPlayerDefaultProps = {
  source: {
...
export const VideoJSPlayer = (props: VideoJsPlayerProps = defaultProps) => {
  const videoRef = React.useRef<HTMLVideoElement>(null);
  let propsCopy: VideoJsPlayerProps = { ...props };
  if (!propsCopy.source) propsCopy.source = defaultProps.source;
  if (!propsCopy.methods) propsCopy.methods = defaultProps.methods;
  if (!propsCopy.position) propsCopy.position = defaultProps.position;
  if (!propsCopy.control) propsCopy.control = defaultProps.control;
  const options: VideoJsPlayerOptions = {
    ...
    bigPlayButton:
      !propsCopy.control.hidePlayButton && propsCopy.source.videoType,
  ...
  const VideoHtml = (top: number, left: number) => (
    <div
      data-vjs-player
      style={{
        top: top,
